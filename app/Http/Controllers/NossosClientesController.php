...
{
    function pagina(){
        $data_view['clientes-banner'] = ModelClientes::orderBy('id', 'desc')->where('foto_fundo','!=',"NULL")->take(5)->get(['id','foto_fundo','logo','nome_pessoa','nome_empresa','resumo_depoimento']);
        $data_view['clientes'] = ModelClientes::all(['id','logo','link','nome_empresa']);
        return view("nossos-clientes")->with('data_view',$data_view);
    }

    function depoimentos($id){
        $clientes = ModelClientes::where('id','=',$id)->get(['logo','titulo_depoimento','foto_fundo','depoimento','nome_empresa']);
        $data_view['cliente-id'] = $id;
        ...
        $data_view['clientes'] = ModelClientes::where('id','!=',$id)->get(['id','logo','link','nome_empresa']);
